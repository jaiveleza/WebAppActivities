			...
			// this is for create calendra and view add event and view event and number of Events
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $current_Month_First_Day+1 || $current_Month_First_Day == 7) && $cb <= ($total_Days_ofMonthDisplay)){
						...
						$result = $db->query("SELECT title FROM events WHERE date = '".$currentDate."' AND status = 1 AND room_id='".$room_id."'");
...
function get_events_information($date = ''){
	...
	if($result->num_rows > 0){
		...
		while($row = $result->fetch_assoc()){
            ...
            $eventListHTML .= '<li>'.'&nbsp'.$row['title'].'&nbsp'.'<a class="del_design" href="delete.php?id='.$row['id'].'&rid='.$room_id.'">DELETE</a>'.'</li>'; // edited
            $eventListHTML .= '<input type="hidden" value="'.$row['id'].' name="delete_event">';
            $eventListHTML .= '<br/>';
        }
		$eventListHTML .= '</ul>';
		$eventListHTML .= '</div>';
...
function add_event_information($date,$title){
	...
	$insert = $db->query("INSERT INTO events (title,date,created,modified,room_id) VALUES ('".$title."','".$date."','".$currentDate."','".$currentDate."','".$room_id."')");
	if($insert){
		echo 'ok';
	}else{
		echo 'err';
